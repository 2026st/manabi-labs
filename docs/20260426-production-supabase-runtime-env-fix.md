...
- `wrangler.toml` の `[assets]` に `run_worker_first = true` を追加し、既存アセット(`"/"`)でもWorkerを必ず通すようにした。
...
- `run_worker_first` が `false` (デフォルト) の場合、`/` は静的アセット優先でWorkerを通らないため置換が走らない。`true` にすると `"/"` も `worker.mjs` を通る。
